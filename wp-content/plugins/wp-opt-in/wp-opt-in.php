...
{
	echo '<form action="#"  name="newsletter" id="newsletter" class="form-horizontal form-inverse">' . "\n";
	?>
	<fieldset class="form-vertical">
		<div class="control-group">
			<div class="controls">
			<input type="text" class="input-block-level required email" placeholder="<?php echo wpoi_get_option('wpoi_form_email');?>" name="wpoi_email" id="wpoi_email" >
			</div>
			<br/>
			<div class="controls">
				<input type="checkbox" class="input-block-leve" name="wpoi_remove" id="wpoi_remove" > <?php _e('Απεγγραφή απο τη λίστα', 'ma-ellak'); ?>
			</div>
		</div>
	 
	<div class="control-group">
		<div class="controls">
		<button class="btn btn-large btn-block btn-inverse newsletter-btn-submit" id="sumbitnewsletter" type="submit"><?php  echo wpoi_get_option('wpoi_form_send'); ?></button>
	</div>
	</div>
	</fieldset>
...
//		$headers .= "Content-Type: text/html; charset=\"" . get_settings('blog_charset') . "\"" . $lf;
		...
		if (!preg_match("/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/", $email)) {
			...
			if(isset($_POST['wpoi_remove']) and strlen($_POST['wpoi_remove']) > 0 and trim($_POST['wpoi_remove']) == 'yes'){ // DELETE USER
			
					$count_exists = "SELECT COUNT(*) as cnt FROM " . $table_users ." where `email` ='".$email . "'";
					
					if( $wpdb->get_var( $count_exists ) != 0){
					
						$delete_member = "DELETE FROM " . $table_users . " WHERE email = '" . $email . "'";
						$result_del = $wpdb->query($delete_member);
						
						$subject_del = '[Μονάδες Αριστείας] Απεγγραφή απο το Newsletter';
						$message_del = 'Η απεγγραφή σας απο τις λίστες ενημέρωσης των Μονάδων Αριστείας ήταν επιτυχής. Αυτό είναι ένα αυτόματο μήνυμα, μην απαντήσετε.';
						
						if (mail($email, $subject_del, $message_del, $headers)) {
							$wpoi_submit_status = "<div class='alert alert-success'><a class='close' data-dismiss='alert'>×</a><strong>Επιτυχής!</strong>
						<p><b>Η απεγγραφή σας είναι επιτυχής. Σας εχει αποσταλλεί ενημερωτικό μήνυμα.</b></p></div>";
							
						} else {
							$wpoi_submit_status = "<div class='alert alert-success'><a class='close' data-dismiss='alert'>×</a><strong>Επιτυχής!</strong>
						<p><b>Η απεγγραφή σας είναι επιτυχής.</b></p></div>";
							
						} 
					} else {
						$wpoi_submit_status = "<div class='alert alert-warning'><a class='close' data-dismiss='alert'>×</a><strong>Πρόβλημα!</strong><p><b>To email δεν εντοπίστηκε.</b></p> </div>";
					}
			} else { // ADD USER
				...
				if (mail($email, $subject, $message, $headers)) {
					$wpoi_submit_status = "<div class='alert alert-success'><a class='close' data-dismiss='alert'>×</a><strong>Επιτυχής!</strong>". stripslashes(wpoi_get_option('wpoi_msg_sent')) ."</div>";
					
				} else {
					$wpoi_submit_status = "<div class='alert alert-warning'><a class='close' data-dismiss='alert'>×</a><strong>Επιτυχής!</strong>". stripslashes(wpoi_get_option('wpoi_msg_sent')). stripslashes(wpoi_get_option('wpoi_msg_fail')) ."</div>";
