	...
	*/
	...
	function pv_show_video($url,$post_id,$mode='post'){
		...
		if (preg_match('/\b(youtube)\.com\b/i', $url)){
			...
			$return = '<object type="application/x-shockwave-flash" style="width:'.$width.'px; height:'.$height.'px;" data="'.$url.'"><param name="movie" value="'.$url.'" /><param name="wmode" value="transparent"></param></object>';			
		}
		else if (preg_match('/\b(google)\.com\b/i', $url)){
			//EMBED GOOGLE VIDEO
			$return ='<embed style="width:'. $width .'px; height:'. $height .'px;" id="VideoPlayback" align="middle" type="application/x-shockwave-flash" src='. $url .' allowScriptAccess="sameDomain" quality="best" bgcolor="#ffffff" scale="noScale" wmode="window" salign="TL"  FlashVars="playerMode=embedded"> </embed>';
		}
		else if (preg_match('/\b(vimeo)\.com\b/i', $url)){
			//EMBED VIMEO VIDEO
			$video_id=explode(".com/", $url);
			$return ='<object width="'. $width .'" height="'. $height .'"><param name="allowfullscreen" value="true" /><param name="allowscriptaccess" value="always" /><param name="movie" value="http://vimeo.com/moogaloop.swf?clip_id='. $video_id[1] .'&amp;server=vimeo.com&amp;color=00adef&amp;fullscreen=1" /><embed src="http://vimeo.com/moogaloop.swf?clip_id='. $video_id[1] .'&amp;server=vimeo.com&amp;color=00adef&amp;fullscreen=1" type="application/x-shockwave-flash" allowfullscreen="true" allowscriptaccess="always" width="'. $width .'" height="'. $height .'"></embed>
						</object>';
		}
		return $return;
	}
	...
	function pv_footer(){
		if(is_single()){
			...
			$pv = get_post_meta($post->ID,'pv_video',true);
			if(!empty($pv)){
				echo '<p align="center"><a href="http://www.spotonseoservices.com/embed-youtube-videos-in-wordpress-plugin/">YouTube Videos in WordPress Plugin</a> by <a href="http://videoproductionhoustontexas.com">Video Production Houston</a></p>';
			}
		}
	}
?>