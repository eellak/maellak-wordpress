 ...
 * @since 1.2
 */
function bp_docs_has_docs( $args = array(), $level='' ) {
	global $bp, $wp_query;
	...
	if ( empty( $bp->bp_docs->doc_query ) ) {
		...
		/**ma-ellak**/
		/** Εκτύπωση μόνο όσων wikis έχουν parent=0 -- δηλαδή δεν έχουν κανένα γονιό***/
		if ($level==0)
			add_filter( 'posts_where', 'ma_ellak_wikis_post_content', 10, 2 );
		/***********************/
...
/**
* Φίλτρο το οποίο χρησιμοποιείται για να κάνει display μόνο όσα wikis δεν έχουν κανένα γονιό
* Είσοδος:
...
*/
function ma_ellak_wikis_post_content( $where, &$wp_query ){
    global $wpdb;
